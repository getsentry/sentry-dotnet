...
| `LNK1322` cannot avoid potential ARM hazard (Cortex-A53 MPCore processor bug #843419) | `.github/workflows/build.yml` | Native AOT for .NET 10+ requires Visual Studio 2026, and VS 2022's linker can fail win-arm64 publishes with this error ([dotnet/runtime#133835](https://github.com/dotnet/runtime/issues/133835)). Run the `.NET (win-arm64)` job on a runner image with VS 2026 (`windows-11-vs2026-arm`). Leave `build-sentry-native` alone: the `sentry-native.lib` it builds ships in the package, and a newer MSVC would stop older linkers from consuming it. |
