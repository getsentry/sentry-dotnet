...
internal class SentrySqlListener : IObserver<KeyValuePair<string, object?>>
{
    ...
    private static void SetDatabaseAddress(ISpanTracer span, string databaseAddress)
    {
        ...
        span.SetData(OTelKeys.DbServer, databaseAddress);
    }
    ...
    private static void SetConnectionId(ISpanTracer span, Guid? connectionId)
    {
        ...
        span.SetData(SqlKeys.DbConnectionId, connectionId);
    }
    ...
    private static void SetOperationId(ISpanTracer span, Guid? operationId)
    {
        ...
        span.SetData(SqlKeys.DbOperationId, operationId);
    }

    private static Guid? TryGetOperationId(ISpanTracer span) => span.Data.TryGetValue<string, Guid?>(SqlKeys.DbOperationId);

    private static Guid? TryGetConnectionId(ISpanTracer span) => span.Data.TryGetValue<string, Guid?>(SqlKeys.DbConnectionId);
    ...
    private void AddSpan(string operation, object? value)
    {
        ...
        span.SetData(OTelKeys.DbSystem, "sql");
    ...
    private void FinishCommandSpan(object? value, SpanStatus spanStatus)
    {
        ...
        if (TryGetConnectionSpan(transaction!, connectionId) is { } connectionSpan)
        {
            if (connectionSpan.Data.TryGetValue<string, string>(OTelKeys.DbName) is { } dbName)
            {
    ...
    private static void TrySetConnectionStatistics(ISpanTracer span, object? value)
    {
        ...
        {
            span.SetData("rows_sent", selectRows);
        }

        if (statistics["BytesReceived"] is long bytesReceived)
        {
            span.SetData("bytes_received", bytesReceived);
        }

        if (statistics["BytesSent"] is long bytesSent)
        {
            span.SetData("bytes_sent ", bytesSent);
        }
    }
}
