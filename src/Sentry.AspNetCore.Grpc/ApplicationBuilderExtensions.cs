...
namespace Microsoft.AspNetCore.Builder
{
    ...
    {
        /// <summary>
        /// Use Sentry gRPC integration
        /// </summary>
        /// <param name="app">The application.</param>
        /// <returns></returns>
        public static IApplicationBuilder UseSentryGrpc(this IApplicationBuilder app)
        {
            // Container is built so resolve a logger and modify the SDK internal logger
            var options = app.ApplicationServices.GetService<IOptions<SentryAspNetCoreGrpcOptions>>();
            if (options?.Value is { } o)
            {
                if (o.Debug && (o.DiagnosticLogger == null ||
                                o.DiagnosticLogger.GetType() == typeof(ConsoleDiagnosticLogger)))
                {
                ...
                {
                    o.AddExceptionProcessorProvider(app.ApplicationServices
                        .GetServices<ISentryEventExceptionProcessor>);
                }
            }
