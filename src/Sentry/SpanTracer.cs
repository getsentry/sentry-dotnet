...
/// </summary>
public class SpanTracer : ITransactionTracer
{
    ...
    internal SpanTracer? ParentSpan { get; }

    internal SpanTracer RootSpan => ParentSpan?.RootSpan ?? this;
    ...
    /// </summary>
    public SpanTracer(IHub hub, ITransactionContext context) : this(hub, context, null)
    {
    }
    ...
    /// Initializes an instance of <see cref="SpanTracer"/>.
    /// </summary>
    internal SpanTracer(IHub hub, string name, string operation, TransactionNameSource nameSource = TransactionNameSource.Custom)
    {
        _hub = hub;
    ...
    /// Initializes an instance of <see cref="SpanTracer"/>.
    /// </summary>
    internal SpanTracer(IHub hub, ITransactionContext context, TimeSpan? idleTimeout = null)
    {
        _hub = hub;
        _options = _hub.GetSentryOptions();
        Name = context.Name;
        ...
        if (context is SpanTracer spanTracer)
        {
            ParentSpan = FindParentSpan(spanTracer, ParentSpanId);
        }
        ...
        string operation)
    {
        ...
        ParentSpan = FindParentSpan(rootSpan, parentSpanId);
        Name = rootSpan.Name;
    ...
    private static SpanTracer? FindParentSpan(SpanTracer parentSpan, SpanId? parentSpanId)
    {
        if (!parentSpanId.HasValue)
        {
            return null;
        }
        if (parentSpan.RootSpan.SpanId == parentSpanId.Value)
        {
            return parentSpan.RootSpan;
        }
        foreach (var ancestor in parentSpan.RootSpan.Spans)
        {
            if (ancestor is SpanTracer ancestorTracer && ancestorTracer.SpanId == parentSpanId)
            {
                return ancestorTracer;
            }
        }
        return null;
    }
