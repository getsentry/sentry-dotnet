...
internal sealed class DefaultSentryStructuredLogger : SentryStructuredLogger
{
    ...
    internal DefaultSentryStructuredLogger(IHub hub, SentryOptions options, ISystemClock clock)
    {
        ...
        _batchProcessor = new BatchProcessor(hub, ClampBatchCount(options.Experimental.InternalBatchSize), ClampBatchInterval(options.Experimental.InternalBatchTimeout), _options.ClientReportRecorder, _options.DiagnosticLogger);
    }
