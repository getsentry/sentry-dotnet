...
interface SentryMXCallStack
{
    ...
    // -(instancetype _Nonnull)initWithThreadAttributed:(BOOL)threadAttributed rootFrames:(NSArray<SentryMXFrame *> * _Nonnull)rootFrames __attribute__((objc_designated_initializer));
    [Export ("initWithThreadAttributed:rootFrames:")]
    [DesignatedInitializer]
    NativeHandle Constructor (bool threadAttributed, SentryMXFrame[] rootFrames);
}
...
interface SentryMXCallStackTree
{
    ...
    // -(instancetype _Nonnull)initWithCallStacks:(NSArray<SentryMXCallStack *> * _Nonnull)callStacks callStackPerThread:(BOOL)callStackPerThread __attribute__((objc_designated_initializer));
    [Export ("initWithCallStacks:callStackPerThread:")]
    [DesignatedInitializer]
    NativeHandle Constructor (SentryMXCallStack[] callStacks, bool callStackPerThread);

    // +(SentryMXCallStackTree * _Nullable)fromData:(NSData * _Nonnull)data error:(NSError * _Nullable * _Nullable)error __attribute__((warn_unused_result("")));
    [Static]
    [Export ("fromData:error:")]
    [return: NullAllowed]
    SentryMXCallStackTree FromData (NSData data, [NullAllowed] out NSError error);
}
...
interface SentryMXFrame
{
    ...
    // @property (readonly, copy, nonatomic) NSArray<SentryMXFrame *> * _Nonnull frames;
    [Export ("frames", ArgumentSemantic.Copy)]
    SentryMXFrame[] Frames { get; }

    // @property (readonly, copy, nonatomic) NSArray<SentryMXFrame *> * _Nonnull framesIncludingSelf;
    [Export ("framesIncludingSelf", ArgumentSemantic.Copy)]
    SentryMXFrame[] FramesIncludingSelf { get; }
}
...
interface SentryVideoInfo
{
    ...
    // @property (readonly, nonatomic) NSInteger frameCount;
    [Export ("frameCount")]
    nint FrameCount { get; }

    // @property (readonly, nonatomic) NSInteger frameRate;
    [Export ("frameRate")]
    nint FrameRate { get; }
    ...
    // -(instancetype _Nonnull)initWithPath:(NSURL * _Nonnull)path height:(NSInteger)height width:(NSInteger)width duration:(NSTimeInterval)duration frameCount:(NSInteger)frameCount frameRate:(NSInteger)frameRate start:(NSDate * _Nonnull)start end:(NSDate * _Nonnull)end fileSize:(NSInteger)fileSize __attribute__((objc_designated_initializer));
    [Export ("initWithPath:height:width:duration:frameCount:frameRate:start:end:fileSize:")]
    [DesignatedInitializer]
    NativeHandle Constructor (NSUrl path, nint height, nint width, double duration, nint frameCount, nint frameRate, NSDate start, NSDate end, nint fileSize);
}
...
{
    // @property (readonly, copy, nonatomic, class) NSString * _Nonnull SENSITIVE_DATA_SUBSTITUTE;
    [Static]
    [Export ("SENSITIVE_DATA_SUBSTITUTE")]
    string SENSITIVE_DATA_SUBSTITUTE { get; }
