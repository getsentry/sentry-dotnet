...
- The `Extra` property and `SetExtra` method on `Span`, `Scope` and `Transaction` have been renamed to `Data` and `SetData` for consistency with Sentry's other SDKs. You'll need to update any references to these properties/methods in your own code. ([#2727](https://github.com/getsentry/sentry-dotnet/pull/2727))
